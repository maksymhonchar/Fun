...
public class Algorithms {
	...
	public void PrintTriangleMatrix() {
		...
		for (int i = 0; i < this.dimension; i++) {
			for (int j = 0; j < this.dimension; j++) {
				...
				if (triangleMatrix[i][j] == INFINITY) {
					System.out.printf("%5s", "inf");
					continue;
				}
				System.out.printf("%5d", triangleMatrix[i][j]);
			}
			System.out.println("");
		}
		System.out.println("");
	...
	public void index() {
		...
		this.FulkersonSequence = new int[this.dimension];
		int count = 0;
		int size = this.dimension;
		for (int l = 0; l < this.dimension;) {
			index: for (int i = 0; i < this.dimension; i++) {
				count = 0;
				for (int j = 0; j < this.dimension; j++) {
					if (newMatrix[j][i] == INFINITY || newMatrix[j][i] == 0) {
						count++;
					}
				}
				if (count == size) {
	...
	public void PrintMaxDistances() {
		...
		for (int i = 0; i < this.FulkersonMax.length; i++) {
			System.out.println("Max path to " + (this.FulkersonSequence[i] + 1) + ": " + this.FulkersonMax[i]);
		}
		System.out.println("");
	}
}
