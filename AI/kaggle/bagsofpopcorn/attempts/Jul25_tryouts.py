...
# Try out different TextBlob classifiers

# .classify 

# textblob_nb_clf_rawreview = NaiveBayesClassifier(textblob_train_data_rawreview[:1000])
# textblob_nltk_clf_rawreview = NLTKClassifier(textblob_train_data_rawreview[:1000])
...
textblob_nb_clf_clnreview = NaiveBayesClassifier(textblob_train_data_clnreview[:5000])

print('textblob_nb_clf_clnreview - trained')

# textblob_nb_clf_rawreview.classify(train_df.loc[0, 'review'])

# NOTE: not enough memory to fill up all 25k entries
# skip this approach for now

# todo: # NaiveBayesClassifier, NLTKClassifier, DecisionTreeClassifier
...
score = textblob_nb_clf_clnreview.prob_classify(test_df.loc[0, 'review'])


round(score.prob("neg"), 2)
...
test_df['cleaned_review'] = test_df['review'].apply(
    lambda x: clean_review(
        x,
        to_lower=True, lemmatize=True, remove_numbers=True, remove_stopwords=True
    )
)
...
# clean review:
    # lemmatize
        # https://www.kaggle.com/jingbinxu/imdb-review-deep-model-93-51-accuracy-c935d8
    # no numbers / with numbers
    # no stopwords / with stopwords
    # with and without lower for vader / for textblob

# use tools from crypto-sentiment-app: textblob
...
# try out NB
# https://www.kaggle.com/luckyboy1220/movie-sentiment-analysis-using-beautifulsoup

# try tf-idf + theory about it
# https://www.kaggle.com/devsanjay/bag-of-words-meets-bag-of-popcorn
# https://www.kaggle.com/dajiaoyang/kernele51f4e5b6e
